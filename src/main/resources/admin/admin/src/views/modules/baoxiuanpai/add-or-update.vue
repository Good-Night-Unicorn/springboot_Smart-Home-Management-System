	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info'&& !ro.baoxiushipin" label="报修视频" prop="baoxiushipin" >
					<file-upload
						tip="点击上传报修视频"
						action="file/upload"
						:limit="1"
						:type="2"
						:multiple="true"
						:fileUrls="ruleForm.baoxiushipin?ruleForm.baoxiushipin:''"
						@change="baoxiushipinUploadChange"
					></file-upload>
				</el-form-item> 
				<el-form-item v-else-if="ruleForm.baoxiushipin" label="报修视频" prop="baoxiushipin" >
					<el-button class="viewBtn" type="text" size="small" @click="download($base.url+ruleForm.baoxiushipin)">
				...
				<el-form-item class="select" v-if="type!='info'"  label="接单状态" prop="jiedanzhuangtai" >
					<el-select :disabled="ro.jiedanzhuangtai" v-model="ruleForm.jiedanzhuangtai" placeholder="请选择接单状态" >
						<el-option
							v-for="(item,index) in jiedanzhuangtaiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="接单状态" prop="jiedanzhuangtai" >
					<el-input v-model="ruleForm.jiedanzhuangtai"
						placeholder="接单状态" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item v-if="type!='info'"  label="报修原因" prop="baoxiuyuanyin" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.baoxiuyuanyin" label="报修原因" prop="baoxiuyuanyin" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.baoxiuyuanyin"></span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'){
					...
					for (var o in obj){
						...
						if(o=='baoxiushipin'){
							this.ruleForm.baoxiushipin = obj[o];
							this.ro.baoxiushipin = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
						...
						if(o=='jiedanzhuangtai'){
							this.ruleForm.jiedanzhuangtai = obj[o];
							this.ro.jiedanzhuangtai = true;
							continue;
						}
					}
					this.ruleForm.jiedanzhuangtai = '待接单'; 				}
				// 获取用户信息
				this.$http({
				...
				this.$http({
					url: `option/weixiuyuan/weixiuzhanghao`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
				...
				this.jiedanzhuangtaiOptions = "已接单,待接单".split(',')
			
			},
			// 下二随
			weixiuzhanghaoChange () {
				this.$http({
					url: `follow/weixiuyuan/weixiuzhanghao?columnValue=`+ this.ruleForm.weixiuzhanghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.baoxiuyuanyin = this.ruleForm.baoxiuyuanyin.replace(reg,'../../../springbootx2vu108k/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					if(this.ruleForm.baoxiushipin!=null) {
						this.ruleForm.baoxiushipin = this.ruleForm.baoxiushipin.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `baoxiuanpai/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.baoxiuanpaiCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			baoxiushipinUploadChange(fileUrls) {
				this.ruleForm.baoxiushipin = fileUrls;
			},
		}
	};
</script>
