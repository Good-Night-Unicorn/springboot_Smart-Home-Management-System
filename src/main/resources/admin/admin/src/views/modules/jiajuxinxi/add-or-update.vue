	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="家居分类" prop="jiajufenlei" >
					<el-select :disabled="ro.jiajufenlei" v-model="ruleForm.jiajufenlei" placeholder="请选择家居分类" >
						<el-option
				...
				<el-form-item v-else class="input" label="家居分类" prop="jiajufenlei" >
					<el-input v-model="ruleForm.jiajufenlei"
						placeholder="家居分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="库存数量" prop="shuliang" >
					<el-input v-model.number="ruleForm.shuliang" placeholder="库存数量" clearable  :readonly="ro.shuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="库存数量" prop="shuliang" >
					<el-input v-model="ruleForm.shuliang" placeholder="库存数量" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-if="type!='info'"  label="家居详情" prop="jiajuxiangqing" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.jiajuxiangqing" label="家居详情" prop="jiajuxiangqing" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.jiajuxiangqing"></span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				this.$http({
					url: `option/jiajufenlei/jiajufenlei`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.jiajufenleiOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jiajuxiangqing = this.ruleForm.jiajuxiangqing.replace(reg,'../../../springbootx2vu108k/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jiajuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiajuxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
