	...
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-else-if="ruleForm.pingjianeirong" label="评价内容" prop="pingjianeirong" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.pingjianeirong"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.pingjianeirong = this.ruleForm.pingjianeirong.replace(reg,'../../../springbootx2vu108k/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `baoxiupingjia/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.baoxiupingjiaCrossAddOrUpdateFlag = false;
											this.parent.search();
