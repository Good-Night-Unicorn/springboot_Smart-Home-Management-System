		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">家居名称：</div>
					<el-input v-model="formSearch.jiajumingcheng" placeholder="家居名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">家居风格：</div>
					<el-input v-model="formSearch.jiajufengge" placeholder="家居风格" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">家居材料：</div>
					<el-input v-model="formSearch.jiajucailiao" placeholder="家居材料" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('jiajuxinxi','新增')" type="primary" @click="add('/index/jiajuxinxiAdd')">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.jiajutupian.split(',')[0])" v-if="item.jiajutupian && item.jiajutupian.substr(0,4)=='http'" :src="item.jiajutupian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.jiajutupian?item.jiajutupian.split(',')[0]:''))" v-else :src="baseUrl + (item.jiajutupian?item.jiajutupian.split(',')[0]:'')" />
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('jiajufenlei/list',{params: {sort: 'id',order: 'asc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'jiajufenlei'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.indexQueryCondition) {
					searchWhere.jiajumingcheng = '%' + this.indexQueryCondition + '%';
					this.indexQueryCondition = '';
				}
				if (this.formSearch.jiajumingcheng != '') searchWhere.jiajumingcheng = '%' + this.formSearch.jiajumingcheng + '%';
				if (this.formSearch.jiajufengge != '') searchWhere.jiajufengge = '%' + this.formSearch.jiajufengge + '%';
				if (this.formSearch.jiajucailiao != '') searchWhere.jiajucailiao = '%' + this.formSearch.jiajucailiao + '%';
				if (this.formSearch.pinpai != '') searchWhere.pinpai = '%' + this.formSearch.pinpai + '%';
				if (this.curFenlei != '全部') searchWhere.jiajufenlei = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jiajuxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiajuxinxiDetail', query: params});
			},
