<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="接单时间" prop="jiedanshijian">
				<el-date-picker
			...
			<el-form-item class="add-item"  label="维修状态" prop="weixiuzhuangtai">
				<el-select v-model="ruleForm.weixiuzhuangtai" placeholder="请选择维修状态" :disabled=" false  ||ro.weixiuzhuangtai" >
					<el-option
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`baoxiujiedan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`baoxiujiedan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
