	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/baoxiujiedan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">接单时间</div>
						<div class="text "  >{{detail.jiedanshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">维修状态</div>
						<div class="text "  >{{detail.weixiuzhuangtai}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('baoxiujiedan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('baoxiujiedan','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('baoxiujiedan','反馈')" 
							@click="onAcross('baoxiufankui','','','weixiuzhuangtai','已维修','已维修,待维修'.split(',')[0])" type="warning">
							反馈
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/baoxiujiedan', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/baoxiujiedanAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此报修接单？') .then(_ => {
					this.$http.post('baoxiujiedan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
