<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="房间名称" prop="fangjianmingcheng">
			...
			<el-form-item class="add-item" label="家居位置" prop="jiajuweizhi">
				<el-input v-model="ruleForm.jiajuweizhi" 
					placeholder="家居位置" clearable :disabled=" false  ||ro.jiajuweizhi"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="控制时间" prop="kongzhishijian">
				<el-date-picker
					:disabled=" false  ||ro.kongzhishijian"
					value-format="yyyy-MM-dd HH:mm:ss"
					v-model="ruleForm.kongzhishijian" 
					type="datetime"
					placeholder="控制时间">
				</el-date-picker>
			</el-form-item>
			<el-form-item class="add-item"  label="控制操作" prop="kongzhicaozuo">
				<el-select v-model="ruleForm.kongzhicaozuo" placeholder="请选择控制操作" :disabled=" false  ||ro.kongzhicaozuo" >
					<el-option
						v-for="(item,index) in kongzhicaozuoOptions"
						:key="index"
						:label="item"
						:value="item">
					</el-option>
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.kongzhishijian = this.getCurDateTime()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='kongzhishijian'){
							this.ruleForm.kongzhishijian = obj[o];
							this.ro.kongzhishijian = true;
							continue;
						}
						if(o=='kongzhicaozuo'){
							this.ruleForm.kongzhicaozuo = obj[o];
							this.ro.kongzhicaozuo = true;
							continue;
						}
					}
				}else if(type=='edit'){
				...
				this.kongzhicaozuoOptions = "开,关".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`yuanchengkongzhi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yuanchengkongzhi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
