	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/baoxiuanpai?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">维修账号</div>
						<div class="text "  >{{detail.weixiuzhanghao}}</div>
					</div>
					<div class="item">
						<div class="lable">维修姓名</div>
						<div class="text "  >{{detail.weixiuxingming}}</div>
					</div>
					<div class="item">
						<div class="lable">接单状态</div>
						<div class="text "  >{{detail.jiedanzhuangtai}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('baoxiuanpai','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('baoxiuanpai','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('baoxiuanpai','接单')" 
							@click="onAcross('baoxiujiedan','','','jiedanzhuangtai','已接单','已接单,待接单'.split(',')[0])" type="warning">
							接单
						</el-button>
					</div>
				</div>
			...
			<video v-if="detail.baoxiushipin" ref="baoxiushipin" :src="baseUrl +  detail.baoxiushipin"  controls="controls"  controlsList="nodownload">
				您的浏览器不支持视频播放
			</video>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card">
				<el-tab-pane label="报修原因" name="first">
					<div class="ql-snow ql-editor" v-html="detail.baoxiuyuanyin"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`baoxiuanpai`);
			...
			backClick(){
				...
				this.$router.push({path: '/index/baoxiuanpai', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/baoxiuanpaiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此报修安排？') .then(_ => {
					this.$http.post('baoxiuanpai/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
