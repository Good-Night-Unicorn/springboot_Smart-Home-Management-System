	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="家居分类" prop="jiajufenlei">
				<el-input v-model="ruleForm.jiajufenlei" 
					placeholder="家居分类" clearable :disabled=" false  ||ro.jiajufenlei"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="报修视频" v-if="type!='cross' || (type=='cross' && !ro.baoxiushipin)" prop="baoxiushipin">
				<file-upload
					tip="点击上传报修视频"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" v-else label="报修视频" prop="baoxiushipin">
				<div class="viewBtn" v-if="ruleForm.baoxiushipin" @click="download(baseUrl + ruleForm.baoxiushipin)" type="success">预览</div>
			...
			<el-form-item class="add-item" label="维修账号" prop="weixiuzhanghao">
				<el-select  @change="weixiuzhanghaoChange" v-model="ruleForm.weixiuzhanghao" placeholder="请选择维修账号">
					<el-option
			...
			<el-form-item class="add-item"  label="接单状态" prop="jiedanzhuangtai">
				<el-select v-model="ruleForm.jiedanzhuangtai" placeholder="请选择接单状态" :disabled=" false  ||ro.jiedanzhuangtai" >
					<el-option
			...
			<el-form-item class="add-item" label="报修原因" prop="baoxiuyuanyin">
				<editor 
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/weixiuyuan/weixiuzhanghao', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.weixiuzhanghaoOptions = res.data.data;
					}
				});
			...
			weixiuzhanghaoChange () {
				this.$http.get('follow/weixiuyuan/weixiuzhanghao?columnValue=' + this.ruleForm.weixiuzhanghao, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						if(res.data.data.weixiuxingming){
							this.ruleForm.weixiuxingming = res.data.data.weixiuxingming
						}
					}
				});
			},
			...
			// 多级联动参数
			info() {
				this.$http.get(`baoxiuanpai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`baoxiuanpai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			baoxiushipinUploadChange(fileUrls) {
				this.ruleForm.baoxiushipin = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
