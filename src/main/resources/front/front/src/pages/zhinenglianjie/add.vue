<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="家居状态" prop="jiajuzhuangtai">
				<el-select v-model="ruleForm.jiajuzhuangtai" placeholder="请选择家居状态" :disabled=" false  ||ro.jiajuzhuangtai" >
					<el-option
			...
			<el-form-item class="add-item" label="wifi连接" prop="wuxian">
				<el-input v-model="ruleForm.wuxian" 
					placeholder="wifi连接" clearable :disabled=" false  ||ro.wuxian"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="参数值" prop="canshuzhi">
				<el-input v-model="ruleForm.canshuzhi" 
					placeholder="参数值" clearable :disabled=" false  ||ro.canshuzhi"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`zhinenglianjie/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zhinenglianjie/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
